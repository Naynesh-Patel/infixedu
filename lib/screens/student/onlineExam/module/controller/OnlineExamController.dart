...
import 'package:Saksham/screens/student/onlineExam/module/controller/ExamController.dart';
...
class OnlineExamController extends GetxController
  ...
  void startController(TakeExamModel quizParam) {
    ...
    if (quiz.value.onlineExam?.durationType == "question") {
      questionTime.value = quiz.value.onlineExam?.defaultQuestionTime ?? 0;
    } else {
      questionTime.value = quiz.value.onlineExam?.duration ?? 0;
    }
    ...
    print(types);
    currentQuestion.value = quiz.value.examQuestions?.first ?? ExamQuestion();
      ...
      if (response.statusCode == 200) {
        ...
        Utils.getStringValue('id').then((value) async {
          ...
          await _examController.getAllActiveExam(
              id, userController.selectedRecord.value.id ?? 0);
        });
  ...
  void updateTheQnNum(int index) {
    currentQuestion.value = quiz.value.examQuestions?[index] ?? ExamQuestion();
    checkSelected(index);
  ...
  void questionSelect(index) {
    ...
    _pageController?.animateToPage(index,
        curve: Curves.easeInOut, duration: const Duration(milliseconds: 200));
    if (quiz.value.onlineExam?.durationType == "question") {
  ...
  Future skipPress(index) async {
    ...
    if (_questionNumber.value != quiz.value.examQuestions?.length) {
      currentQuestion.value = quiz.value.examQuestions?[index + 1] ?? ExamQuestion();
